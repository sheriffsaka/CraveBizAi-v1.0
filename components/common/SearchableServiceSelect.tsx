    ...
    <div className={`relative w-full ${className}`} ref={containerRef}>
      ...
      {!isOpen && selectedService ? (
        <div
          ...
          className={`group flex items-center justify-between w-full p-3 border rounded-lg bg-gray-50 text-gray-900 font-bold shadow-sm transition-all cursor-pointer hover:border-primary-400 focus:ring-2 focus:ring-primary-500 outline-none ${
            disabled ? 'opacity-60 cursor-not-allowed' : ''
          }`}
        >
          <div className="flex items-center gap-2.5 overflow-hidden pr-2">
            <div className="w-8 h-8 rounded-md bg-primary-100 text-primary-700 flex items-center justify-center shrink-0 font-extrabold text-xs">
                ...
                >
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 flex-wrap mb-1">
                      ...
                      {service.packageName && (
                        <span className="inline-flex items-center gap-1 text-[10px] font-bold px-2 py-0.5 rounded-full bg-indigo-50 text-indigo-700 border border-indigo-100 shrink-0">
                          <Package className="w-2.5 h-2.5" />
                      ...
                      {service.category && (
                        <span className="inline-flex items-center gap-1 text-[10px] font-bold px-2 py-0.5 rounded-full bg-gray-100 text-gray-600 shrink-0">
                          <Tag className="w-2.5 h-2.5" />
                    ...
                    {service.description && (
                      <p className="text-xs text-gray-500 font-normal line-clamp-2 mt-1">
                  ...
                  <div className="text-right shrink-0 flex flex-col items-end gap-1">
                    <span className="font-black text-sm text-primary-700 bg-primary-50 px-2.5 py-1 rounded-md border border-primary-100/50">
                ...
                {allowCustomItem && (
                  <div className="pt-2">
                    <button
                      ...
                      className="inline-flex items-center justify-center gap-2 px-4 py-2 bg-primary-600 text-white rounded-lg text-xs font-bold hover:bg-primary-700 transition-colors shadow-sm"
                    >
                      <Plus className="w-3.5 h-3.5" />
                      Use "{searchTerm || 'custom'}" as custom line item
                    </button>
                  </div>
                )}
              </div>
          ...
          {allowCustomItem && filteredServices.length > 0 && (
            <div className="p-2 bg-gray-50 border-t border-gray-100">
              <button
                type="button"
                onClick={() => handleCustomSelect()}
                className="w-full py-2 px-3 text-left text-xs font-bold text-gray-600 hover:text-primary-700 hover:bg-white rounded-lg transition-all flex items-center gap-2 border border-dashed border-gray-200"
              >
