...
export default function DocSignify({ company, user, prefillProject, prefillClient, initialFile, onBackToDashboard }: DocSignifyProps) {
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto pb-12">
      ...
      {activeTab === 'wizard' && (
        <div className="space-y-6">
          {/* Step Progress Bar - click any step name to jump there without losing your work */}
          <div className="bg-white p-4 rounded-2xl border border-slate-200/80 shadow-sm">
            <div className="flex items-center justify-between text-xs font-extrabold text-slate-400 max-w-2xl mx-auto">
              <button
                type="button"
                onClick={() => {
                  if (wizardStep === 'complete') return; // dispatched envelope is final - use "Sign Another Document" instead
                  setWizardStep('upload');
                }}
                disabled={wizardStep === 'complete'}
                className={`flex items-center gap-2 ${wizardStep === 'complete' ? 'cursor-default' : 'cursor-pointer hover:opacity-80'} transition-opacity`}
              >
              ...
              {/* Step 3 is the post-dispatch confirmation, not an editable page,
                  so it's only ever reached by actually sending - not by clicking ahead. */}
